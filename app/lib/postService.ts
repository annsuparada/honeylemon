...
import { mapPrismaPostToBlogPost } from "./mappers/postMapper";
...
export async function getPublishedPosts(
    ...
    const posts: BlogPost[] = rawPosts.map(mapPrismaPostToBlogPost);
...
export async function getFeaturedPostByCountry(countrySlug: string): Promise<BlogPost | null> {
    ...
    if (featuredPosts.length > 0) {
        return mapPrismaPostToBlogPost(featuredPosts[0]);
    }
    ...
    return mapPrismaPostToBlogPost(countryPosts[0]);
}
