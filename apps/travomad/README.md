...
Honey Lemon is a full-featured travel blog platform designed for content creators and travel enthusiasts. It combines a beautiful, responsive frontend with a powerful content management system, ensuring optimal SEO performance and excellent user experience.
...
This project is part of an npm workspaces monorepo: the app root is **`apps/travomad`**. Clone the repo, `cd` into it, run **`npm install`** at the **repository root**, then **`npm run dev`** from the root (or `npm run dev -w @honeylemon/travomad`). See the root **`README.md`** for layout and Vercel root directory (`apps/travomad`).
...
│   │   ├── readTime-helpers.ts        # Re-exports `@honeylemon/server/blog/read-time` (client-safe)
...
│   ├── services/         # Thin facades → `@honeylemon/server/blog` factories + prisma
│   │   ├── postService.ts
│   │   ├── userService.ts
│   │   └── categoryService.ts
│   ├── middleware/       # Re-exports `@honeylemon/server/http`
│   │   └── errorHandler.ts
...
│   │   ├── promptBuilder.ts
│   │   └── aiToPostMapper.ts
│   └── services/         # `emailService` wires `@honeylemon/server/email` + app config
│       ├── emailService.ts
│       └── contentAssembler.ts
...
1. **Separation of Concerns**:
   - Many `app/api/**/route.ts` files are **thin shims** that wire `prisma` + `jwtConfig` into factories from [`@honeylemon/server`](../../docs/SERVER_PACKAGE.md) (`routes/post`, `routes/user`, `routes/images`, etc.).
   - Shared HTTP, JWT, email, and Prisma-backed CMS logic lives in **`packages/server`**; this app keeps env and the Prisma client.

2. **Type Safety**:
   - Domain-specific types in `app/types/`
   - Zod schemas in `schemas/` (re-exported from `@honeylemon/cms` where applicable)

3. **Error Handling**:
   - `lib/middleware/errorHandler.ts` re-exports `@honeylemon/server/http` (`handleError`, `successResponse`, custom errors).

4. **Configuration**:
   - `lib/config.ts` supplies secrets and URLs to route factories and `emailService`; nothing secret is imported from `packages/server` at build time.

**Shared server helpers (full guide):** [`docs/SERVER_PACKAGE.md`](../../docs/SERVER_PACKAGE.md).

Monorepo-wide architecture: [`docs/ARCHITECTURE.md`](../../docs/ARCHITECTURE.md).
...
```env
# Database
DATABASE_URL="mongodb://localhost:27017/honeylemon"
...
Honey Lemon implements comprehensive SEO best practices:
...
Honey Lemon uses Cloudinary for image management:
