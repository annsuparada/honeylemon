...
describe('metadata-helpers', () => {
  ...
  describe('getOpenGraphImages', () => {
    ...
    it('uses default image when imageUrl is not provided', () => {
      ...
      expect(result[0].alt).toBe('Honey Lemon');
    });
  ...
  describe('getBaseOpenGraph', () => {
    it('returns correct Open Graph base properties', () => {
      const result = getBaseOpenGraph(
        'Test Title',
        'Test Description',
        'https://honeylemon.com/blog/test'
      );

      expect(result).toEqual({
        siteName: 'Honey Lemon',
        locale: 'en_US',
        url: 'https://honeylemon.com/blog/test',
        title: 'Test Title',
        description: 'Test Description',
      });
    });
    ...
    it('handles empty strings', () => {
      const result = getBaseOpenGraph('', '', 'https://honeylemon.com');
  ...
  describe('generatePostMetadata', () => {
    ...
    it('generates correct metadata for a published post', async () => {
      ...
      expect(result.title).toBe('Test Post Title | Honey Lemon');
      ...
      expect(result.publisher).toBe('Honey Lemon');
    ...
    it('uses default description when post description is missing', async () => {
      ...
      expect(result.description).toBe('Discover travel tips, destination guides, and exclusive deals on Honey Lemon');
    });
