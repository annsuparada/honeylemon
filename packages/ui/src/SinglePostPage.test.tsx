...
describe('SinglePostPage Component', () => {
  ...
  it('renders post not found when post is null', () => {
    render(<SinglePostPage {...defaultProps} post={null} />);
  ...
  it('renders breadcrumb with correct items', () => {
    ...
    expect(breadcrumb.textContent).toMatch(/Blog/);
    expect(breadcrumb.textContent).toMatch(/Test Post Title/);
  });
  ...
  it('renders edit post button and share button', () => {
    render(<SinglePostPage {...defaultProps} />);
    expect(screen.getByTestId('edit-post-button')).toHaveAttribute('href', '/write?slug=test-post');
  ...
  it('renders FAQ section when FAQs are present', () => {
    ...
    expect(screen.getAllByText('What is this about?').length).toBeGreaterThan(0);
  });
  ...
  it('renders blog section with related posts', () => {
    ...
    expect(blogSection).toHaveTextContent('Check out these posts');
  });
  ...
  it('uses correct route prefix in breadcrumb', () => {
    ...
    const breadcrumb = screen.getByTestId('breadcrumb');
    expect(breadcrumb.textContent).toMatch(/Destinations/);
    expect(breadcrumb.textContent).toMatch(/Test Post Title/);
  });
  ...
  it('does not render table of contents when there are no headings and no FAQs', () => {
    const postNoToc = {
      ...mockPost,
      faqs: [],
      content: '<p>Content with no headings in the markup.</p>',
    };
    render(<SinglePostPage {...defaultProps} post={postNoToc} />);
